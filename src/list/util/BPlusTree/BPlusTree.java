...
//            if (node.isLeaf()) {
            //如果是叶子节点维护前后指针
            rightNode = new Node(null, rightKeyAndValues, node.getNextNode(), null, node.getParentNode());
            leftNode = new Node(null, leftKeyAndValues, rightNode, node.getPreviousNode(), node.getParentNode());
        ...
        if (isMoreElement(bratherNode)) {
            ...
            switch (nodeType) {
                case PRENODE:
                    ...
                    keyAndValue1 = getKeyAndValueinMinAndMax(node.getParentNode(), keyAndValue.getKey(), getMinKeyInNode(node));
                    keyAndValue1.setKey(keyAndValue.getKey());
                    break;
                case NEXTNODE:
                    keyAndValue = bratherKeyAndValues.remove(0);
                    keyAndValue1 = getKeyAndValueinMinAndMax(node.getParentNode(), getMaxKeyInNode(node), keyAndValue.getKey());
